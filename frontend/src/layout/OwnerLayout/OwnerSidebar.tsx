...
const ownerNavItems: NavItem[] = [
  ...
  },
  {
    ...
    name: 'Bookings',
    subItems: [
      { name: 'Upcoming Appointments', path: '/owner/upcoming-bookings' },
      { name: 'Booking History', path: '/owner/bookings-history' },
    ],
  },
  {
    icon: <TbReportMoney />,
...
const OwnerSidebar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    ownerNavItems.forEach((nav, index) => {
      if (nav.subItems) {
      ...
      {ownerNavItems.map((nav, index) => (
        <li key={nav.name}>
          ...
          {nav.subItems && (isExpanded || isHovered || isMobileOpen) && (
            <div
              ref={(el) => {
                subMenuRefs.current[`owner-${index}`] = el;
              }}
              className='overflow-hidden transition-all duration-300'
              style={{
                height:
                  openSubmenu === index
                    ? `${subMenuHeight[`owner-${index}`]}px`
                    : '0px',
              }}
            >
