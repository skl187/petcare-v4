...
const services = ["Check-up", "Vaccination", "Surgery Consult", "Diagnostics", "Other"];

const VeterinaryBookingsForm: React.FC<Props> = ({ booking, onSubmit, onCancel }) => {
  const { register, handleSubmit, watch, reset, formState: { isSubmitting } } = useForm<VetBookingFormData>({
  ...
  return (
    <div className="p-4 mx-auto max-w-3xl md:p-6">
      <FormCard title={booking ? "Update Booking" : "New Booking"} onClose={onCancel}>
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="text-sm font-medium">Pet Name *</label>
              <input className="mt-1 w-full px-3 py-2 border rounded-md" {...register("petName", { required: true })} />
            </div>
            <div>
              <label className="text-sm font-medium">Owner Name *</label>
              <input className="mt-1 w-full px-3 py-2 border rounded-md" {...register("ownerName", { required: true })} />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea className="mt-1 w-full px-3 py-2 border rounded-md" rows={3} {...register("notes")} />
          </div>
