...
import ServiceListForm, { ServiceFormData } from "../../../../pages/VetPageForms/ServiceListForm/ServiceListForm";
...
const mockServices: VetService[] = [
  { id: 1, name: "Vaccination", description: "Basic vaccine shot", price: 50, duration: 20, status: "Active" },
  { id: 2, name: "General Check-up", description: "Routine health check", price: 40, duration: 30, status: "Active" },
  { id: 3, name: "Dental Cleaning", description: "Teeth cleaning service", price: 120, duration: 60, status: "Inactive" },
];
...
export default function ServiceListTable() {
  ...
  const { sortedData, requestSort, sortConfig } = useSort(filtered, { key: "id", direction: "asc" } as any);

  const indexOfLast = currentPage * itemsPerPage;
  const currentItems = (sortedData as VetService[]).slice(indexOfLast - itemsPerPage, indexOfLast);
  ...
  const handleApplyAction = () => {
    if (actionDropdown === "Delete") {
      ...
      setRows(prev => prev.map(r => selectedRows.includes(r.id) ? { ...r, status: statusUpdate as ServiceStatus } : r));
      setSelectedRows([]);
    }
  };
  ...
  const confirmEdit = (data: ServiceFormData) => {
    setRows(prev => prev.map(r => r.id === editRow?.id ? { ...r, ...data, price: Number(data.price), duration: Number(data.duration), } : r));
    setEditRow(null);
  };
  ...
  return (
    <div className="p-4 bg-white rounded-xl shadow-md">
                  ...
                  <TableRow key={r.id} className="hover:bg-gray-50">
                    ...
                    <TableCell className="p-2">
                      <div className="flex items-center gap-2">
                        <Badge size="sm" color={r.status === "Active" ? "success" : "error"}>{r.status}</Badge>
                        <Switch
                          label=""
                          checked={r.status === "Active"}
                          onChange={() => {
                            setRows(prev => prev.map(x => x.id === r.id ? { ...x, status: x.status === "Active" ? "Inactive" : "Active" } : x));
                          }}
                        />
                      </div>
                    </TableCell>
                    <TableCell className="p-2">
                      <button onClick={() => handleEdit(r)} className="text-blue-600 hover:text-blue-800 mr-2">
                        <MdEdit />
                      </button>
      ...
      {(isAddDialogOpen || editRow) && (
        <ServiceListForm
          service={editRow || undefined}
          onSubmit={editRow ? confirmEdit : confirmAddNew}
          onCancel={() => { setIsAddDialogOpen(false); setEditRow(null); }}
        />
      )}
