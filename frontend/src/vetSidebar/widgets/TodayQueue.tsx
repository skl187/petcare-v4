...
export default function TodayQueue({ data }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/50 p-5">
      <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-3">Today's Queue</h3>
      {todayAppts.length > 0 ? (
            ...
            <li key={appt.id} className="py-3 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  {appt.appointment_type} • Owner: {appt.user_first_name} {appt.user_last_name}
                </div>
              </div>
              <span className={`text-xs px-2.5 py-1 rounded-full ${getBadgeClasses(appt.status)}`}>
