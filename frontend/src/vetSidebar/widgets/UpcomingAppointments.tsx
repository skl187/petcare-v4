      ...
      {upcomingAppts.length > 0 ? (
        <div className="space-y-3">
          {upcomingAppts.map((appt) => (
            <div key={appt.id} className="rounded-xl border border-gray-200 dark:border-gray-700 p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="text-sm font-medium text-gray-800 dark:text-white">
                  {formatDate(appt.appointment_date)} • {formatTime(appt.appointment_time)}
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400">{appt.appointment_type}</div>
              </div>
              <div className="mt-1 text-sm text-gray-600 dark:text-gray-300">
                <span className="font-semibold">{appt.pet_name}</span> — Owner: {appt.user_first_name} {appt.user_last_name}
              </div>
              {appt.clinic_name && (
