...
-- PAYMENTS
-- ============================================================================
-- Single source of truth: vet_appointment_payments stores all payment records
-- Multiple payment records allowed per appointment for split payments
-- vet_payment_transactions stores provider transaction IDs and details
...
CREATE TABLE IF NOT EXISTS vet_appointment_payments (
    ...
    is_partial boolean NOT NULL DEFAULT FALSE,  -- TRUE if this is one of multiple payments for the appointment
    payment_sequence int,                        -- Order sequence for partial payments (1, 2, 3...)
    
    -- For tracking when all partial payments are completed
    split_payment_group_id text,                 -- Groups all partial payments for the same appointment
...
CREATE INDEX IF NOT EXISTS idx_vet_payments_is_partial ON vet_appointment_payments (is_partial);
CREATE INDEX IF NOT EXISTS idx_vet_payments_payment_status ON vet_appointment_payments (payment_status);
CREATE INDEX IF NOT EXISTS idx_vet_payments_split_group ON vet_appointment_payments (split_payment_group_id);
...
CREATE TABLE IF NOT EXISTS vet_payment_transactions (
    ...
    -- Provider transaction IDs (from payment gateways)
    transaction_id text,                         -- Provider's unique transaction/reference ID
    provider_code text,                          -- Provider code (e.g., 'stripe', 'razorpay', 'paypal')
...
CREATE INDEX IF NOT EXISTS idx_payment_transactions_transaction_id ON vet_payment_transactions (transaction_id);
CREATE INDEX IF NOT EXISTS idx_payment_transactions_provider ON vet_payment_transactions (provider_code);
CREATE INDEX IF NOT EXISTS idx_payment_transactions_status ON vet_payment_transactions (status);

-- ============================================================================
-- PAYMENT HELPER FUNCTIONS & VIEWS
-- ============================================================================

-- Function to calculate total paid amount for an appointment across all split payments
CREATE OR REPLACE FUNCTION get_appointment_total_paid(p_appointment_id uuid)
...
-- Function to get split payment status for an appointment
CREATE OR REPLACE FUNCTION get_appointment_payment_status(p_appointment_id uuid)
RETURNS TABLE(
    ...
    SELECT 
        (SELECT COALESCE(MAX(total_amount), 0.00) FROM vet_appointment_payments WHERE appointment_id = p_appointment_id LIMIT 1) as total_amount,
        COALESCE(SUM(paid_amount), 0.00) as total_paid,
        COALESCE(SUM(paid_amount), 0.00) >= COALESCE((SELECT MAX(total_amount) FROM vet_appointment_payments WHERE appointment_id = p_appointment_id), 0.00) as is_fully_paid,
...
-- View to show split payment summary per appointment
CREATE OR REPLACE VIEW v_appointment_split_payments AS
    SELECT 
        ap.appointment_id,
        ap.user_id,
        COUNT(ap.id) as total_payments,
        SUM(CASE WHEN ap.is_partial THEN 1 ELSE 0 END) as partial_payment_count,
        COUNT(DISTINCT ap.split_payment_group_id) as payment_groups,
        SUM(ap.paid_amount) as total_paid,
        ap.total_amount,
        (SUM(ap.paid_amount) >= ap.total_amount) as is_fully_paid,
        CASE 
            WHEN COUNT(ap.id) > 1 THEN 'split_payment'
            ELSE 'single_payment'
        END as payment_type,
        MAX(ap.payment_date) as last_payment_date,
        ap.payment_status
    FROM vet_appointment_payments ap
    GROUP BY ap.appointment_id, ap.user_id, ap.total_amount, ap.payment_status
...
/* Notification infrastructure: channels, user preferences, tokens, notifications, templates */

-- Channels / Integrations (e.g., sms, email, push, whatsapp)
CREATE TABLE IF NOT EXISTS notification_channels (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    slug text NOT NULL UNIQUE,           -- e.g. 'sms','email','push','whatsapp'
    name text NOT NULL,
    provider text,                       -- e.g. 'twilio','sendgrid','fcm'
    config jsonb,                        -- provider-specific configuration (omit secrets in DB in prod or encrypt)
    is_active boolean NOT NULL DEFAULT TRUE,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_notification_channels_slug ON notification_channels (lower(slug));

-- User notification preferences
CREATE TABLE IF NOT EXISTS user_notification_preferences (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    notification_key text NOT NULL, -- e.g. 'vaccination_due','appointment_reminder'
    channel text NOT NULL,          -- 'sms','email','push','whatsapp','in_app'
    enabled boolean NOT NULL DEFAULT TRUE,
    lead_time interval,             -- time before event to trigger reminder (e.g. '7 days')
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
-- User push tokens for FCM/APNs/web push
...
-- Notifications queue table (pending/sent/failed)
CREATE TABLE IF NOT EXISTS notifications (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    notification_key text NOT NULL,
    channel text NOT NULL,
    target jsonb,                     -- { email, phone, push_token }
    template_key text,
    locale text,
    payload jsonb,                    -- variables used to render template
...
-- Notification delivery logs (detailed)
CREATE TABLE IF NOT EXISTS notification_logs (
    ...
    status text NOT NULL CHECK (status IN ('sent','delivered','failed','undeliverable')),
    response jsonb,
    created_at timestamptz NOT NULL DEFAULT now()
);
...
-- Templates + locale-aware translations
CREATE TABLE IF NOT EXISTS notification_templates (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    template_key text NOT NULL UNIQUE,   -- e.g. 'vaccination_due_email'
    name text,
    channel text NOT NULL CHECK (channel IN ('email','sms','push','whatsapp','in_app')),
    description text,
    default_subject text,
    default_body text,                   -- plain text w/ placeholders like {{pet_name}}
...
CREATE TABLE IF NOT EXISTS notification_template_translations (
    ...
    locale text NOT NULL,                -- 'en', 'en-US', 'fr', etc.
    subject text,
    body text,
    body_html text,
    variables jsonb,
...
CREATE INDEX IF NOT EXISTS idx_notification_templates_key ON notification_templates (lower(template_key));
CREATE INDEX IF NOT EXISTS idx_notification_template_translations_locale ON notification_template_translations (locale);

-- Helper function to pick best translation for a template and locale
CREATE OR REPLACE FUNCTION get_notification_template(p_key text, p_locale text)
RETURNS TABLE(
    template_id uuid,
    template_key text,
    channel text,
...
-- SETTINGS
-- ============================================================================
-- Superadmin-level settings for integrations (keyed config)
CREATE TABLE IF NOT EXISTS app_settings (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    namespace text NOT NULL DEFAULT 'global', -- allow grouping multi-tenant / environment
    key text NOT NULL,
    value jsonb,
    description text,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_settings_key_namespace ON app_settings (namespace, lower(key));
