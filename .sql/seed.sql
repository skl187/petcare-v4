...
-- Seed common channels (idempotent)
INSERT INTO notification_channels (slug, name, provider)
VALUES
...
-- Example templates (idempotent)
INSERT INTO notification_templates (template_key, name, channel, default_subject, default_body, default_body_html, variables)
VALUES
('vaccination_due_email', 'Vaccination Due (Email)', 'email',
 'Your pet {{pet_name}} vaccination is due on {{due_date}}',
 'Hello {{owner_name}}, your pet {{pet_name}} is due for {{vaccine_name}} on {{due_date}}. Please schedule an appointment: {{appointment_url}}',
 '<p>Hello {{owner_name}},</p><p>Your pet <strong>{{pet_name}}</strong> is due for <strong>{{vaccine_name}}</strong> on <strong>{{due_date}}</strong>. Please <a href="{{appointment_url}}">schedule an appointment</a>.</p>',
 jsonb_build_object('owner_name','Owner name','pet_name','Pet name','vaccine_name','Vaccine name','due_date','Next due date','appointment_url','Scheduling URL')
)
ON CONFLICT (template_key) DO NOTHING;

INSERT INTO notification_templates (template_key, name, channel, default_body, variables)
VALUES
('appointment_reminder_sms', 'Appointment Reminder (SMS)', 'sms',
 'Reminder: {{pet_name}} has an appointment on {{appointment_date}} at {{appointment_time}}. Clinic: {{clinic_name}}. {{appointment_url}}',
 jsonb_build_object('pet_name','Pet name','appointment_date','Date','appointment_time','Time','clinic_name','Clinic name','appointment_url','Appointment link')
)
ON CONFLICT (template_key) DO NOTHING;


-- Seed default translations (English and a French example)
INSERT INTO notification_template_translations (template_id, locale, subject, body, body_html, is_default)
SELECT id, 'en', default_subject, default_body, default_body_html, TRUE
FROM notification_templates
WHERE template_key = 'vaccination_due_email'
ON CONFLICT DO NOTHING;

INSERT INTO notification_template_translations (template_id, locale, body, is_default)
SELECT id, 'en', default_body, TRUE
FROM notification_templates
WHERE template_key = 'appointment_reminder_sms'
ON CONFLICT DO NOTHING;

INSERT INTO notification_template_translations (template_id, locale, subject, body, body_html, is_default)
SELECT id, 'fr', 'Votre animal {{pet_name}} doit être vacciné le {{due_date}}', 'Bonjour {{owner_name}}, votre animal {{pet_name}} doit recevoir le vaccin {{vaccine_name}} le {{due_date}}. Veuillez prendre rendez-vous: {{appointment_url}}', '<p>Bonjour {{owner_name}},</p><p>Votre animal <strong>{{pet_name}}</strong> doit recevoir le vaccin <strong>{{vaccine_name}}</strong> le <strong>{{due_date}}</strong>. Veuillez <a href="{{appointment_url}}">prendre rendez-vous</a>.</p>', FALSE
FROM notification_templates
WHERE template_key = 'vaccination_due_email'
ON CONFLICT DO NOTHING;

-- Add update triggers
SELECT create_updated_at_trigger('notification_channels');
SELECT create_updated_at_trigger('project_settings');
SELECT create_updated_at_trigger('user_notification_preferences');
SELECT create_updated_at_trigger('user_push_tokens');
SELECT create_updated_at_trigger('notifications');
SELECT create_updated_at_trigger('notification_templates');
SELECT create_updated_at_trigger('notification_template_translations');
SELECT create_updated_at_trigger('notification_logs');
